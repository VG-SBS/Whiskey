...
class MyMainWindow(QMainWindow):
    ...
    def set_qr_value(self):
        text, ok = QInputDialog.getText(self, "", "Bitte URL eingeben")
        if ok and text:
            self.__my_label.generate_qr(text)

    def save_file(self):
        file_name, _ = QFileDialog.getSaveFileName(self, "Bild speichern", "./", "Images (*.png *.jpg)")
        print(file_name)
        if file_name:
